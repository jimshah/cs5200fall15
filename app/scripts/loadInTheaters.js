...
function addMovieToTheatre(movie){
	try {
		return new Promise(function(resolve, reject){
			var query = "insert into theatre (id, title, overview, popularity, poster_path, release_date) values (";
...
function getMoviesInTheatre(page) {
	try {
		return new Promise(function(resolve, reject){
			//return resolve([]);
			page = page || 1;
			var options = {
				...
				path: "/3/movie/now_playing?api_key="+api.config.appKey+"&page="+page,
				method: 'GET'
			};
